...
describe("ElementService", () => {
	...
	describe("initialize", () => {
		it("should initialize repository and load initial state", async () => {
			...
			mockElementRepository.loadInitialState.mockResolvedValue(
				mockElements
			);
	...
	describe("createElement", () => {
		it("should add a new element and save it to the repository", async () => {
			...
			expect(mockElementRepository.saveElement).toHaveBeenCalledWith(
				newElement
			);
		});
	});
	...
	describe("updateElement", () => {
		...
		it("should not update if element does not exist", async () => {
			...
			await expect(
				elementService.updateElement(nonExistentElement)
			).rejects.toThrow("Element not found");

			expect(elementService.getElements()).not.toContainEqual(
				nonExistentElement
			);
	...
	describe("deleteElement", () => {
		it("should remove an element and delete it from the repository", async () => {
			...
			expect(mockElementRepository.deleteElement).toHaveBeenCalledWith(
				elementIdToDelete
			);
		});
		...
		it("should not delete if element does not exist", async () => {
			...
			await expect(
				elementService.deleteElement(nonExistentElementId)
			).rejects.toThrow("Element not found");
